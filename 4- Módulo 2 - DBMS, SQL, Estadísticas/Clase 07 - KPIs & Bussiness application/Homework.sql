...
INSERT INTO alumnos (ID,CédulaIdentidad,Nombre,Apellido,FechaNacimiento,FechaIngreso,Cohorte)
...
INSERT into prueba.instructor (Cédula, Nombre, Apellido, FechaNacimiento, FechaIncorporación) 
...
insert into prueba.cohorte (ID, Código, Carrera, Instructor, FechaInicio,  FechaFin)
...
insert into prueba.cohorte (Código, Carrera, Instructor, FechaInicio,  FechaFin)
VALUES ('FT-1235',1,1,'2020-2-1','2020-6-30')


insert into prueba.alumnos (ID, CédulaIdentidad, Nombre, Apellido, FechaNacimiento, FechaIngreso, Cohorte)
...
(101,'465763558','Gloria','Mcclure','1991-07-02','2022-01-29',1240),
 ...
 #2)
 delete from prueba.cohorte where ID = 1245
  delete from prueba.cohorte where ID = 1246
  
#3)
update prueba.cohorte
set FechaInicio = '2023-05-16'
where ID = 1243

#4)
update prueba.alumnos
set Apellido = 'Ramirez'
where ID = 165

#5)
select Nombre, Apellido, FechaIngreso, Cohorte from prueba.alumnos
where Cohorte = 1243

#6)
select distinct instructor.Nombre, instructor.Apellido, carrera.Nombre from cohorte
join instructor on instructor.ID = cohorte.Instructor 
join carrera on carrera.ID = cohorte.Carrera
where carrera.Nombre = 'Full Stack Developer'

#7)
select Nombre, Apellido, Cohorte from alumnos
where Cohorte = 1235
and year(FechaIngreso) = 2019

#8)
select Nombre, Apellido, Cohorte, FechaIngreso from alumnos
where FechaIngreso between '2019-01-01' and '2019-12-31'


#9) Varias formas de filtrar los mismos registros.
SELECT alumnos.Nombre, Apellido, FechaNacimiento, carrera.nombre
FROM alumnos
INNER JOIN cohorte
ON cohorte.ID = alumnos.Cohorte
INNER JOIN carrera
ON carrera.ID = cohorte.Carrera
...
select FechaIngreso, concat(Nombre,' ',Apellido) as Nombre_Completo from alumnos 
...
select FechaIngreso from alumnos 
ORDER BY FechaIngreso aSC
limit 1;
...
#8. La función YEAR le permite extraer el año de un campo date, utilice esta función y especifique 
#cuantos alumnos ingresarona a Henry por año.
select year(FechaIngreso) as año, count(ID) as cantidad_de_alumnos from alumnos
GROUP BY year(FechaIngreso);

#9. ¿Cuantos alumnos ingresaron por semana a henry?, indique también el año. WEEKOFYEAR()
select weekofyear(FechaIngreso) as Semana, year(FechaIngreso) as año, count(ID) as cantidad_de_alumnos from alumnos
...
#11. Investigue las funciones TIMESTAMPDIFF() y CURDATE(). ¿Podría utilizarlas para saber cual 
#es la edad de los instructores?. 
#¿Como podrías verificar si la función cálcula años completos? Utiliza DATE_ADD().
select Nombre, Apellido, FechaNacimiento, curdate() as Fecha_Hoy, timestampdiff(year,FechaNacimiento,curdate()) as Edad from instructor;
...
#           - La edad promedio de los alumnos de cada cohorte.<br>
 ...
 select Nombre, Apellido, FechaNacimiento, curdate() as Fecha_Hoy, timestampdiff(year,FechaNacimiento,curdate()) as Edad from alumnos;

 
select avg(timestampdiff(year,FechaNacimiento,curdate())) from alumnos;


select Cohorte, avg(timestampdiff(year,FechaNacimiento,curdate())) from alumnos
GROUP BY Cohorte;


#13. Elabora un listado de los alumnos que superan la edad promedio de Henry.
select Nombre, Apellido, timestampdiff(year,FechaNacimiento,curdate()) as edad from alumnos
where timestampdiff(year,FechaNacimiento,curdate()) > (select avg(timestampdiff(year,FechaNacimiento,curdate())) from alumnos)
